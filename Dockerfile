...
# 2. Copiar e instalar el API Gateway (CAMBIADO A wd-api-gateway)
COPY wd-api-gateway ./wd-api-gateway
RUN mvn -s /usr/share/maven/ref/settings.xml -f wd-api-gateway/pom.xml clean package -DskipTests
...
# CAMBIADO A wd-api-gateway
COPY --from=builder /app/wd-api-gateway/target/*.jar app.jar
